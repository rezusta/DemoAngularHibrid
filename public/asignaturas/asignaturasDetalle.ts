(function() {
    ...
    var asignaturasDetalle = {
        //templateUrl: './asignaturas/detalleAsignatura.html',
        templateUrl: './detalleAsignatura.html',
        bindings: {},
        controller: class AsignaturasDetalleCtrl {
            ...
            constructor(servicioAsignaturas, $location, $routeParams, $mdToast) {
                ...
                servicioAsignaturas.getAsignatura($routeParams.ID).then(this.onAsignaturaDetalle, this.onError);
            }
      
            onAsignaturaActualizada = (data) => {
                this.mostrarMensaje("La asignatura se ha actualizado correctamente");
            }
            onAsignaturaDetalle = (data) => {
                this.ID = data[0].ID;
                this.nombre = data[0].nombre;
                this.aula = data[0].aula;
                this.dia = data[0].dia;
                this.hora = data[0].hora;
                this.ano = data[0].ano;
            }
            onError = (reason) => {
            ...
            actualizarAsignatura = (ID, nombre, aula, dia, hora, ano) => {
                this.servicioAsignaturas.putAsignatura(ID, nombre, aula, dia, hora, ano).then(this.onAsignaturaActualizada, this.onErrorCreacion);
            }
            cancelar = () => {
    ...
    app.component("asignaturasDetalle", asignaturasDetalle);
  }());
